...
locals {
  ...
  iops = var.ghes_data_volume_type == "io1" ? var.ghes_data_volume_iops < local.data_volume_size * 3 ? local.data_volume_size * 3 > 32000 ? 32000 : local.data_volume_size * 3 : var.ghes_data_volume_iops : 0
}

resource "aws_security_group" "security_group" {
  # NOTE: https://help.github.com/enterprise/admin/installation/network-ports
...
resource "aws_eip" "ghes_eip" {
  ...
  tags = merge(
    {
      "Name" = format("%s-%d-eip", var.name, count.index + 1)
    },
    var.tags,
  )
}
